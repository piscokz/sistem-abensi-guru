        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('guru-piket.mapel.update', $mapel->id) }}">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="nama_mapel" :value="__('Nama Mapel')" />
                        ...
                        {{-- <div class="mt-4">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="mapel" {{ old('status', $mapel->status) == 'mapel' ? 'selected' : '' }}>Mata Pelajaran
                                </option>
                                <option value="kosong" {{ old('status', $mapel->status) == 'kosong' ? 'selected' : '' }}>Jam Kosong
